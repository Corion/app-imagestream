...
sub generate {
    ...
    for my $image (@items) {
        ...
        my $url_large = join "/", 
                          $base_url,
                          $image->{sizes}->{medium}->{name}->basename
                        ;
        my $url_thumb = join "/", 
                          $base_url,
                          $image->{sizes}->{thumbnail}->{name}->basename
                        ;
        my $title = $image->title;
        my $html = <<HTML; # encode_entities(<<HTML);
<p>$info->{author} hat ein Bild ver&ouml;ffentlicht:</p>
<p><a href="$url_large" title="$title"><img src="$url_thumb" width="$image->{sizes}->{thumbnail}->{width}" height="$image->{sizes}->{thumbnail}->{height}" alt="$title" /></a></p>
HTML
